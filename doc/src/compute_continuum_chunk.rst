...
The simplest way to reproduce the previous workflow is to combine
this compute with :doc:`compute property/chunk <compute_property_chunk>`
for chunk coordinates or original IDs and with :doc:`fix ave/time
<fix_ave_time>` for time averaging and file output, for example:
...
where :math:`m_i` is the atom mass, :math:`\vec{r}_\mathrm{chunk}` is the
chunk center, :math:`\vec{r}_i` is the atom position, and :math:`W` is
the kernel.
...
where :math:`V_i` is the particle volume in 3d or area in 2d.
...
gradients along that dimension are zero.
...
where :math:`v_{\mathrm{chunk},a}` is the :math:`a` component of the
coarse-grained chunk velocity.
