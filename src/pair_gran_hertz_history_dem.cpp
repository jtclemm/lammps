  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq >= radsum*radsum) {
        ...
        dx = (radsum-r)*rinv; //Goes from zero to infinity as r->0
        ccel = kn*dx*exp(C_exp*dx)-damp;
...
void PairGranHertzHistoryDEM::settings(int narg, char **arg)
{
  ...
  dampflag = utils::numeric(FLERR,arg[5],false,lmp);
  if (dampflag == 0) gammat = 0.0;

  C_exp = utils::numeric(FLERR,arg[6],false,lmp);
...
void PairGranHertzHistoryDEM::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
    ...
    utils::sfread(FLERR,&C_exp,sizeof(int),1,fp,NULL,error);
  }
  ...
  MPI_Bcast(&C_exp,1,MPI_INT,0,world);
}
                                    ...
                                    double &fforce)
{
  ...
  double dx = radsum/r-1.0; //Goes from zero to infinity as r->0
  ccel *= exp(C_exp*dx);
  ccel -= damp;
  polyhertz = sqrt((radsum-r)*radi*radj / radsum);
  ccel *= polyhertz;
