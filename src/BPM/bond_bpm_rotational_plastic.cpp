...
#include "bond_bpm_rotational_plastic.h"
...
BondBPMRotationalPlastic::BondBPMRotationalPlastic(LAMMPS *_lmp) :
    BondBPM(_lmp), Kr(nullptr), Ks(nullptr), Kt(nullptr), Kb(nullptr), gnorm(nullptr),
    gslide(nullptr), groll(nullptr), gtwist(nullptr), e_plastic(nullptr), angle_plastic(nullptr), e_break(nullptr), angle_break(nullptr)
...
BondBPMRotationalPlastic::~BondBPMRotationalPlastic()
{
  delete[] svector;
  ...
  if (allocated) {
    ...
    memory->destroy(Kt);
    memory->destroy(Kb);
    memory->destroy(e_plastic);
    memory->destroy(angle_plastic);
    memory->destroy(e_break);
    memory->destroy(angle_break);
...
void BondBPMRotationalPlastic::store_data()
{
  int i, j, m, type;
  double delx, dely, delz, r, rinv;
  double **x = atom->x;
                                         ...
                                         double *force1on2, double *torque1on2, double *torque2on1, double &rmag_eq, double &gamma_eq, double &theta_eq, double &psi_eq)
{
  ...
  // Plastically evolve shear angle
  if (gamma - gamma_eq > angle_plastic[type]) {
    gamma_eq = gamma - angle_plastic[type];
  } else if (gamma - gamma_eq < -angle_plastic[type]) {
    gamma_eq = gamma + angle_plastic[type];
  }
  gamma = gamma - gamma_eq;
  ...
  // Plastically evolve bending/twisting angles
  if (theta - theta_eq > angle_plastic[type]) {
    theta_eq = theta - angle_plastic[type];
  } else if (theta - theta_eq < -angle_plastic[type]) {
    theta_eq = theta + angle_plastic[type];
  }
  theta = theta - theta_eq;

  if (psi - psi_eq > angle_plastic[type]) {
    psi_eq = psi - angle_plastic[type];
  } else if (psi - psi_eq < -angle_plastic[type]) {
    psi_eq = psi + angle_plastic[type];
  }
  psi = psi - psi_eq;
...
void BondBPMRotationalPlastic::damping_forces(int i1, int i2, int type, double *rhat, double *r,
...
void BondBPMRotationalPlastic::allocate()
{
  allocated = 1;
  ...
  memory->create(e_plastic, np1, "bond:e_plastic");
  memory->create(e_break, np1, "bond:e_break");
  memory->create(angle_plastic, np1, "bond:angle_plastic");
  memory->create(angle_break, np1, "bond:angle_break");
...
void BondBPMRotationalPlastic::coeff(int narg, char **arg)
{
  ...
  double e_plastic_one = utils::numeric(FLERR, arg[5], false, lmp);
  double e_break_one = utils::numeric(FLERR, arg[6], false, lmp);
  double angle_plastic_one = utils::numeric(FLERR, arg[7], false, lmp);
  double angle_break_one = utils::numeric(FLERR, arg[8], false, lmp);
  ...
  for (int i = ilo; i <= ihi; i++) {
    ...
    Kt[i] = Kt_one;
    Kb[i] = Kb_one;
    e_plastic[i] = e_plastic_one;
    e_break[i] = e_break_one;
    angle_plastic[i] = angle_plastic_one;
    angle_break[i] = angle_break_one;
    gnorm[i] = gnorm_one;
    gslide[i] = gslide_one;
    groll[i] = groll_one;
    gtwist[i] = gtwist_one;
    ...
    if (1.0 + e_break_one > max_stretch) max_stretch = 1.0 + e_break_one;
  }
...
void BondBPMRotationalPlastic::init_style()
{
  BondBPM::init_style();

  if (!atom->quat_flag || !atom->radius_flag || !atom->omega_flag)
    error->all(FLERR, "Bond bpm/rotational/platic requires atom style bpm/sphere");
  if (comm->ghost_velocity == 0)
    error->all(FLERR, "Bond bpm/rotational/platic requires ghost atoms store velocity");

  if (domain->dimension == 2)
    error->warning(FLERR, "Bond style bpm/rotational/platic not intended for 2d use");
}

/* ---------------------------------------------------------------------- */

void BondBPMRotationalPlastic::settings(int narg, char **arg)
{
...
void BondBPMRotationalPlastic::write_restart(FILE *fp)
{
  ...
  fwrite(&e_plastic[1], sizeof(double), atom->nbondtypes, fp);
  fwrite(&e_break[1], sizeof(double), atom->nbondtypes, fp);
  fwrite(&angle_plastic[1], sizeof(double), atom->nbondtypes, fp);
  fwrite(&angle_break[1], sizeof(double), atom->nbondtypes, fp);
...
void BondBPMRotationalPlastic::read_restart(FILE *fp)
{
  ...
  if (comm->me == 0) {
    ...
    utils::sfread(FLERR, &e_plastic[1], sizeof(double), atom->nbondtypes, fp, nullptr, error);
    utils::sfread(FLERR, &e_break[1], sizeof(double), atom->nbondtypes, fp, nullptr, error);
    utils::sfread(FLERR, &angle_plastic[1], sizeof(double), atom->nbondtypes, fp, nullptr, error);
    utils::sfread(FLERR, &angle_break[1], sizeof(double), atom->nbondtypes, fp, nullptr, error);
  ...
  MPI_Bcast(&e_plastic[1], atom->nbondtypes, MPI_DOUBLE, 0, world);
  MPI_Bcast(&e_break[1], atom->nbondtypes, MPI_DOUBLE, 0, world);
  MPI_Bcast(&angle_plastic[1], atom->nbondtypes, MPI_DOUBLE, 0, world);
  MPI_Bcast(&angle_break[1], atom->nbondtypes, MPI_DOUBLE, 0, world);
...
void BondBPMRotationalPlastic::write_restart_settings(FILE *fp)
{
...
void BondBPMRotationalPlastic::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double BondBPMRotationalPlastic::single(int type, double rsq, int i, int j, double &fforce)
{
