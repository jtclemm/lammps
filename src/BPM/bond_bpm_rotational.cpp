...
static const char cite_bond_bpm_rotational[] =
  "bond bpm/rotational command:\n\n"
  "@Article{Wang09,\n"
  " author =  {Y. Wang},\n"
...
BondBPMRotational::BondBPMRotational(LAMMPS *lmp) : Bond(lmp)
{
  if (lmp->citeme) lmp->citeme->add(cite_bond_bpm_rotational);
...
{
  if(fix_bond_store) modify->delete_fix("BOND_STORE_BPM_ROTATIONAL");
...
void BondBPMRotational::calc_forces(int type, double r_mag, double r0_mag, double *q1, double *q2, double *r0, double *r, double *force1on2, double *torque1on2, double *torque2on1, double &Fs_mag, double &Fr_mag, double &Tb_mag, double &Tt_mag)
{
...
{
  if (!atom->quat_flag)
    error->all(FLERR,"Bond bpm/rotational requires atom attributes quaternion");
  if (comm->ghost_velocity == 0)
    error->all(FLERR,"Bond bpm/rotational requires ghost atoms store velocity");

  if (force->pair == NULL || force->pair->single_enable == 0)
    error->all(FLERR,"Pair style does not support bond_style bpm/rotational");

  if (force->angle || force->dihedral || force->improper)
    error->all(FLERR,
               "Bond style bpm/rotational cannot be used with 3,4-body interactions");
  if (atom->molecular == 2)
    error->all(FLERR,
               "Bond style bpm/rotational cannot be used with atom style template");

  if (atom->special_flag)
    error->all(FLERR, "Special bonds must be turned off for bond style bpm/rotational");

  if(domain->dimension == 2)
    error->warning(FLERR, "Bond style bpm/rotational not intended for 2d use, may be inefficient");

  //Define bond store
  if (fix_bond_store == NULL) {
    char **fixarg = new char*[5];
    fixarg[0] = (char *) "BOND_STORE_BPM_ROTATIONAL";
    fixarg[1] = (char *) "all";
    ...
    delete [] fixarg;
    int ifix = modify->find_fix("BOND_STORE_BPM_ROTATIONAL");
...
double BondBPMRotational::equilibrium_distance(int i)
{
...
void BondBPMRotational::read_restart(FILE *fp)
{
  ...
  if (comm->me == 0) {
    ...
    fread(&Tct[1],sizeof(double),atom->nbondtypes,fp);
    fread(&Tcb[1],sizeof(double),atom->nbondtypes,fp);
...
void BondBPMRotational::write_data(FILE *fp)
{
  for (int i = 1; i <= atom->nbondtypes; i++)
     fprintf(fp,"%d %g %g %g %g %g %g %g %g %g %g %g\n",i,Kr[i],Ks[i],Kt[i],Kb[i],Fcr[i], Fcs[i], Tct[i], Tcb[i], gamma[i],gammaw[i], C_exp[i]);
}
