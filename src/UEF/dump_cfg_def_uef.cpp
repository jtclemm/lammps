...
void DumpCFGDefUef::init_style()
{
  ...
  for (i=0; i<modify->nfix; i++)
  {
    if (strcmp(modify->fix[i]->style,"def/uef")==0) {
      fix_class = DEFORM;
      break;
    } if (strcmp(modify->fix[i]->style,"def/berendsen/uef")==0) {
      fix_class = DEFORM_BERENDSEN;
      break;
    }
  }
  if (i==modify->nfix)
    error->all(FLERR,"Can't use dump cfg/uef without defining a fix def/uef");
  ifix_uef=i;
}
...
void DumpCFGDefUef::write_header(bigint n)
{
